...
describe('ActionCreators', () => {
  ...
  it('\'signin\' should post Users correctly', (done) => {
    const spy = jest.spyOn(axios, 'post').mockImplementation((username, password) => new Promise((resolve) => {
      const result = {
        status: 204,
      };
      resolve(result);
  ...
  it('\'signup\' should post User info correctly', (done) => {
    const spy = jest.spyOn(axios, 'post').mockImplementation((username, currentPassword, newPassword) => new Promise((resolve, reject) => {
      const result = {
        status: 200,
      };
      resolve(result);
  ...
  it('\'changeInfo\' should post User info correctly', (done) => {
    const spy = jest.spyOn(axios, 'put').mockImplementation((username, currentPassword, newPassword) => new Promise((resolve, reject) => {
      const result = {
        status: 200,
      };
      resolve(result);
